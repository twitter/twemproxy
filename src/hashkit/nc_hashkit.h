...
uint32_t hash_rcrc32(const char *key, size_t key_length);
...
rstatus_t redisarray_update(struct server_pool *pool);
uint32_t redisarray_dispatch( uint32_t ncontinuum, uint32_t hash);
